...
std::vector<weight_t> key(N, std::numeric_limits<weight_t>::max());
...
while (!active.empty()) {
    node_t where = active.begin()->second;
...
// #pragma omp parallel for
    for (size_t i = 0; i < graph[where].size(); ++i) {
...
node_t u = 0;
#pragma omp parallel for
for (size_t i = 0; i < toEdges.size(); ++i) {
    node_t v = toEdges[i];
    if (v != INIT) {
...
#pragma omp parallel for reduction(+: postprocessed_route2_cost)
...
#pragma omp parallel for reduction(+: postprocessed_route3_cost)
